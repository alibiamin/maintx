...
export default function PartFamiliesList() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/part-families').then((r) => setList(Array.isArray(r.data) ? r.data : [])).catch(() => snackbar.showError('Erreur chargement')).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);

  const openCreate = () => { setEditingId(null); setForm({ code: '', name: '', description: '' }); setDialogOpen(true); };
  const openEdit = (row) => { setEditingId(row.id); setForm({ code: row.code || '', name: row.name || '', description: row.description || '' }); setDialogOpen(true); };

  const handleSave = () => {
    if (!form.name.trim()) { snackbar.showError('Nom requis'); return; }
    setSaving(true);
    const payload = { ...form, code: form.code || undefined };
    (editingId ? api.put(`/part-families/${editingId}`, payload) : api.post('/part-families', payload))
  ...
  const handleDelete = (id) => {
    ...
    api.delete(`/part-families/${id}`).then(() => { snackbar.showSuccess('Supprimé'); load(); }).catch((e) => snackbar.showError(e.response?.data?.error || 'Erreur'));
  };

  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2} mb={3}>
        <Typography variant="h5" fontWeight={700}>{t('item.part_families')}</Typography>
      ...
      <Card>
        {loading ? <Box p={4} display="flex" justifyContent="center"><CircularProgress /></Box> : (
          <Table>
            ...
            <TableBody>
              {list.length === 0 ? (
                <TableRow><TableCell colSpan={canEdit ? 4 : 3} align="center"><Typography color="text.secondary">Aucune famille</Typography></TableCell></TableRow>
              ) : (
                list.map((row) => (
                  <TableRow key={row.id}>
                    <TableCell>{row.code}</TableCell><TableCell>{row.name}</TableCell><TableCell>{row.description || '—'}</TableCell>
                    {canEdit && (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingId ? 'Modifier' : 'Nouvelle famille'}</DialogTitle>
        <DialogContent>
