...
export default function BrandsList() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/brands').then((r) => setList(Array.isArray(r.data) ? r.data : [])).catch(() => snackbar.showError('Erreur chargement')).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    (editingId ? api.put(`/brands/${editingId}`, form) : api.post('/brands', form))
  ...
  const handleDelete = (id) => {
    ...
    api.delete(`/brands/${id}`).then(() => { snackbar.showSuccess('Supprimé'); load(); }).catch((e) => snackbar.showError(e.response?.data?.error || 'Erreur'));
  };

  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2} mb={3}>
        <Typography variant="h5" fontWeight={700}>{t('item.brands')}</Typography>
      ...
      <Card>
        {loading ? <Box p={4} display="flex" justifyContent="center"><CircularProgress /></Box> : (
          <Table>
            ...
            <TableBody>
              {list.length === 0 ? (
                <TableRow><TableCell colSpan={canEdit ? 4 : 3} align="center"><Typography color="text.secondary">Aucune marque</Typography></TableCell></TableRow>
              ) : (
                list.map((row) => (
                  <TableRow key={row.id}>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingId ? 'Modifier la marque' : 'Nouvelle marque'}</DialogTitle>
        <DialogContent>
